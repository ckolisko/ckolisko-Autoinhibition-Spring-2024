...
def generatePlots(xValues, plotData,
    ...
    plotFileName="plotFile", fileSuffix=".png",
    colorHues=[], colorOffset=0, #max is three.
    specialOpts=[],
    grid=False,
        ...
        for j in range(jLen):
            ...
            for groupInd, group in enumerate(plotData[i][j]):
                ...
                for shadeInd, itm in enumerate(group):
                    ...
                    for curXBreak, xBreak in enumerate(xBreaks):
                        if xBreak[0] >= len(itm):
                            break
                        ...
                        if curXBreak==0:
                            ...
                            addLater.append({
                                "PlotIndex": i*jLen+j+1,
                                "X": xValues[pltStartEnd[0]:pltStartEnd[1]],
                                "Y": itm[pltStartEnd[0]:pltStartEnd[1]],
                                "Color": getVector3FromHueAndBrightness(colorHue, shadeVal)
                            })
                    if grid:
                        plt.grid()
    
    for itm in addLater:
        plt.subplot(iLen, jLen, itm["PlotIndex"])
        plt.plot(itm["X"], itm["Y"], linewidth=2, color=itm["Color"])
    ...
    resultingPath = Path(plotFileName).with_suffix(fileSuffix) # Assemble the full filename + path
    newFig = open(resultingPath,'x') # Attempt to open a new file
    newFig.close() # Close it before saving the figure to this path.

    fig.savefig(resultingPath, bbox_inches='tight', dpi=180, transparent=True)