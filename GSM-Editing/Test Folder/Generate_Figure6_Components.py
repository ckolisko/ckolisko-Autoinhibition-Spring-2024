...
targFolder = Path("P:/PhD Work/Assets/GSM Images/FigurePlots")
...
newGNLow.setInitialConditions(
    #                            5                                  10                                 15
    ["OFF", "OFF", "OFF", "OFF", "BLK", "BLK", "BLK", "BLK", "BLK", "BLK", "BLK", "BLK", "BLK", "BLK", "OFF", "ON"], # Initial annealed conditions of each genelet
    [   50,    25,    10,    25,    30,    50,    50,    20,    10,    20,    50,    50,    50,    10,     5,    5], # Initial concentrations of each genelet
    GSM.createGeneralClassProperties(
        #                   5                         10
        [250, 250, 125, 50, 100, 150, 250, 125,  400, 200, 200], # Activator concentrations (lowest to highest genelet index, ex: the 2 in "G2C1")
        [  0,   0,   0, 20,  70, 100,   0,   0, 2000,   0,   0]  # Blocker concentrations (lowest to highest genelet index)
    )
)

# Makes a bandpass filter that captures medium-strength outputs
...
# Makes a bandpass filter that captures high-strength outputs
...
simTime = 16 # Time in hours that the simulations will run
simTimeVals = GSM.getTimeValues(simTime)
# Convert the active concentrations to total RNA produced by accounting for
# the time (in hours, convert to seconds *3600), the rate constant (0.02 1/s), and the interpolation (1000 frames)
rtVal = simTime/1000*3600*0.02


numOfSims = 101 # Minimum 2, start and end point of activator concentration profile
maxActivator = 330 # (nM) Use A1 concentrations between 0 nM and this value
...
AP.generatePlots(inputVals/max(inputVals), allEndpoints,
    ...
    plotFileName=targFolder / "Figure_6_A_1",
    fixedY=(-0.04, 1.0)
)
...
AP.generatePlots(inputVals/max(inputVals), allDifferences,
    ...
    plotFileName=targFolder / "Figure_6_B_1",
    colorOffset=1.5,
    fixedY=(-30, 570)
)
...
AP.generatePlots(inputVals/max(inputVals), allRS10Prod,
    ...
    plotFileName=targFolder / "Figure_6_NA_1",
    colorOffset=2,
    fixedY=(-90, 2100)
)
...
AP.generatePlots(inputVals/max(inputVals), lowEndPt,
    ...
    plotFileName=targFolder / "Figure_6_C_1",
    fixedY=(-0.04, 1.0)
)
...
AP.generatePlots(inputVals/max(inputVals), medEndPt,
    localXTitle="Input Pulse Size",
    localYTitle="Fraction ON G10R11 @ 11 HR",
    plotFileName=targFolder / "Figure_6_D_1",
    fixedY=(-0.04, 1.0)
)
...
AP.generatePlots(inputVals/max(inputVals), highEndPt,
    localXTitle="Input Pulse Size",
    localYTitle="Fraction ON G10R11 @ 11 HR",
    plotFileName=targFolder / "Figure_6_E_1",
    fixedY=(-0.04, 1.0)
)
...
AP.generatePlots(inputVals/max(inputVals), lowDiff,
    localXTitle="Input Pulse Size",
    localYTitle="Total Activity G5C4 - G5CS4 (nM)",
    plotFileName=targFolder / "Figure_6_C_2",
    colorOffset=1.5,
    fixedY=(-30, 570)
)
...
AP.generatePlots(inputVals/max(inputVals), medDiff,
    localXTitle="Input Pulse Size",
    localYTitle="Total Activity G5C4 - G5CS4 (nM)",
    plotFileName=targFolder / "Figure_6_D_2",
    colorOffset=1.5,
    fixedY=(-30, 570)
)
...
AP.generatePlots(inputVals/max(inputVals), highDiff,
    localXTitle="Input Pulse Size",
    localYTitle="Total Activity G5C4 - G5CS4 (nM)",
    plotFileName=targFolder / "Figure_6_E_2",
    colorOffset=1.5,
    fixedY=(-30, 570)
)
...
AP.generatePlots(inputVals/max(inputVals), lowRS10Prod,
    localXTitle="Input Pulse Size",
    localYTitle="Total Activity G4RS10 (nM)",
    plotFileName=targFolder / "Figure_6_C_3",
    colorOffset=2,
    fixedY=(-90, 2100)
)
...
AP.generatePlots(inputVals/max(inputVals), medRS10Prod,
    localXTitle="Input Pulse Size",
    localYTitle="Total Activity G4RS10 (nM)",
    plotFileName=targFolder / "Figure_6_D_3",
    colorOffset=2,
    fixedY=(-90, 2100)
)
...
AP.generatePlots(inputVals/max(inputVals), highRS10Prod,
    localXTitle="Input Pulse Size",
    localYTitle="Total Activity G4RS10 (nM)",
    plotFileName=targFolder / "Figure_6_E_3",
    colorOffset=2,
    fixedY=(-90, 2100)
)
