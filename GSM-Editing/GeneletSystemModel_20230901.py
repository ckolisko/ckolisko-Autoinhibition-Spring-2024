...
def GenerateGeneletEquations(tVal, dVals,
    ...
    # Add optional enzyme activity dynamics
    if "TempRegulation" in RNAPRegression:
        if "AmtBelow37" in RNAPRegression:
            tempDiff = RNAPRegression["TempRegulation"]["AmtBelow37"]
        else:
            tempDiff = 16
        temp = 1 - tempDiff/310*np.exp(-RNAPRegression["TempRegulation"]["Rate"]*tVal)
        RNAP *= temp*np.exp(-1/temp)
    
    if RNAPRegression["Type"]=="Constant":
        pass
    elif RNAPRegression["Type"]=="ExpDecay":
        RNAP *= np.exp(-RNAPRegression["Params"][0]*tVal)
    elif RNAPRegression["Type"]=="Logistic":
        RNAP *= 1/(1 + np.exp(RNAPRegression["Params"][0]*(tVal - RNAPRegression["Params"][1])))
...
class GeneletNetwork:
    ...
    def simulate(self, timeList, resetRun=True, unitChoice="nM"):
        ...
        startTime = 0
        if not resetRun:
            startTime = self.TimeList[-1]

        rawSolution = spi.solve_ivp(rates, [startTime, timeList[-1]], self.InitialConditions if resetRun else self.CurrentConditions, method="LSODA", t_eval=timeList)
    ...
    def modifyRate(self, rateType, newValue, target=None):
        if rateType=="Production":
            translateBox = {"C":"CoactivatorP", "R":"RepressorP", "CS":"CSilencerP", "RS":"RSilencerP"}
            self.RxnRates[translateBox[target]] = np.full(len(self.OCEs), newValue)
