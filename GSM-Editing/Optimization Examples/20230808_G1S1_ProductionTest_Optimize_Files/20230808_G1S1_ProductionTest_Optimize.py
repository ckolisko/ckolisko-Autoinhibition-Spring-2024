...
# Raw data file name
# Pre: must delete the nums at top of raw data.
fileName = "20230808_G1S1_ProductionTest"
# Raw data path name.
filePath = "C:/Users/kolis/Desktop/Research/GSM-Editing/Optimization Examples/20230808_G1S1_ProductionTest_Optimize_Files/"
wellPlate1 = AA.WellPlate(filePath+fileName+".txt")

# Unsure what this does.
manualAddn = ""

# False if you don't want to find the time bounds (I may have found them).
getNormalized = True

# True if you want only the curve.
hideEarlyData = False
hideLateData = False

# True if flourecence is decreasing in experiment.
invertWellData = False
clrOffset = 2.5 # 1.7 for deep blue, 2.5 for reddish pink

# Look at recipe protocol / raw data file for column names.
wellCoords = AA.getWellNames("K8", "L11")

# Gets the "Time breaks" for the well data. In this data, it is the break from 1140 to 2012 min.
...
if getNormalized:
    ...
    for wellCoord in wellCoords:
        ...
        # Not sure what this does. Normalizes the data.
...
# Putting the correct wells.
# Below is code for graphing.
# wellsGraphSetup = [
#     [
#         [
#             [
...
# TODO fix to recipe.
g1s1Concs = [0, 0.002, 0.004, 0.008, 0.016]
A1Concs = [0, 0.01, 0.02, 0.04, 0.08]

gNs = []

for i, x in enumerate(g1s1Concs):
    gNs.append(GSM.GeneletNetwork(
        #                        5
        # TODO g2- G5S1 genelet, g1 - s1 quencher, g3 - junk
        # Is this okay in this case. When is it not okay?
        ["G2", "G1", "G3"],
        ["C1",   "", ""]
    ))

    gNs[-1].setInitialConditions(
        # G2, G1, G3, matches above order                       5
        # TODO: where are these initial amounts in recipe protocol?
        ["ON", "BLK", "ON"],
        [   5,   500, 100],
