...
for i in range(numGeneletNetworks):
    ...
    #gNs[-1].modifyRate("AutoInhib-Act-Repressor", 10**-3.0283858301562177 , 1) # genelet K_M
    #gNs[-1].modifyRate("AutoInhib-Free-Act", 10**-3.710287279668953, 1) # genelet K_M
    gNs[-1].modifyRate("AutoInhib-Act-Genelet", 10**-3.8000584213581954 , 1) # genelet K_M
    ...
    specialOpts = [
        ["Legend",
            [ # Same principle as the plot4DMat variable, except only 2 layers (row, column) since the names are just in order of what was plotted to each subplot.
                [
                    ["Simulated Well 12", "Simulated Well 13","Real Well 12","Real Well 13",]
                ]
            ]
        ] 
    ],
