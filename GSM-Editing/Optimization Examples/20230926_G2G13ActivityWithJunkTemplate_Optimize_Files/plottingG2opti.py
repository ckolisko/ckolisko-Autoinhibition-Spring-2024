...
for i in range(numGeneletNetworks):
    ...
    #gNs[-1].modifyRate("AutoInhib-Act-Repressor", 10**-3.016192284481294 , 1) # genelet K_M
    #gNs[-1].modifyRate("AutoInhib-Free-Act", 10**-3.8906789871649425, 1) # genelet K_M
    gNs[-1].modifyRate("AutoInhib-Act-Genelet", 10**-3.973631792332391 , 1) # genelet K_M
    ...
    specialOpts = [
        ["Legend",
            [ # Same principle as the plot4DMat variable, except only 2 layers (row, column) since the names are just in order of what was plotted to each subplot.
                [
                    ["Simulated Well 7", "Simulated Well 8","Real Well 7","Real Well 8",]
                ]
            ]
        ] 
    ],
