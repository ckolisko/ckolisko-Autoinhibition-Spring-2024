...
generalRateConstants = {
    ...
    "ActDNARep" : {
        "kVal": 10000*1e-9, # (1/nM/s) Free activator sequestered via DNA repressor
        "List": "OCE"
    },
    "ActDNARepDisplacement" : {
        "kVal": 5000*1e-9, # (1/nM/s) Activator sequestered via DNA repressor strand displacement
        "List": "G"
    },

    "AntiRepressing" : {
        "kVal": 10000*1e-9, # (1/nM/s) Free DNA repressor sequestered via Anti DNA Repressor
        "List": "OCE"
    },
    "AntiRepressDisplace": {
        "kVal": 5000*1e-9, # (1/nM/s) DNA repressor sequestered via Anti DNA repressor strand displacement
        "List": "OCE"
    },
    ...
    "RNaseADegrade": {
        "kVal": 200., # (uL/U RNase A/s) Degradation rate of RNase A (kd_A)
        "List": "OCE"
    },
    
    # Added these to affect dFreeRepress in generate genelet equations.
    "AutoInhib-Act-Repressor":{
         "kVal": 0.0,
         "List": "OCE"
    },
    "AutoInhib-Free-Act":{
...
# Kinetic Rates of enzyme
enzymeProdRates = {
    "RNAPProdRate" : 2., # (nM*uL/U RNAP/s) Assumed maximum production constant
    "K_M"          : 45. # (nM) Assumed enzyme complex equilibrium constant with template (leads to 50% of production if template is this concentration)
}

# Supporting rates of enzyme function over time
enzymeSuppRates = {
    "RNAPHalfLife" : 500., # (s) The half-life of RNAP (in this model, we consider RNAP as getting "revived" by DTT)
    "DTTHalfLife"  : 6000., # (s) The half-life of DTT, a reducing agent that is treated as responsible for extending the lifespan of T7 RNAP.
    "RNAPRevival"  : 1. # (1/nM/s) The expected rate for "reviving" dead RNAP through DTT presence (this is an immense simplification of the denaturing mechanism)
}


# Number of minutes for the temperature to equilibriate by 1 - 1/e (~ 63%)
heatingRate = {
    "CYT5": 11.
}
...
geneletOutputs = ["C","R","CS","RS","AntiR"]
...
def createGeneralClassProperties(actVec, blkVec, DNARepVec=None, aptProteinVec=[], unitChoice="nM"):
    actLen = len(actVec)
    if actLen!=len(blkVec):
        sys.exit("Activator and Blocker lists must be of equal length.")
    elif DNARepVec==None:
        DNARepVec = [0]*actLen
    elif actLen!=len(DNARepVec):
        sys.exit("Activator and DNA Repressor lists must be of equal length.")
    ...
    return {
        "G":{ # Genelets
            ...
            "DNARepressor": np.array(DNARepVec, dtype=np.dtype(float))*unitChoices[unitChoice] # Concentration of DNA repressor in nM
        },
        "A":{ # Aptlets
...
# Creates an array of linearly interpolated time values.
...
# unitResult: The unit of the resulting time values. Should be seconds if to be used in a GSM simulation.
def getTimeValues(maxTime, minTime=0, dataPointNum=1001, unitChoice="hr", unitResult="sec"):
    return np.linspace(minTime, maxTime, dataPointNum)*unitChoices[unitChoice]/unitChoices[unitResult]
...
def GenerateGeneletEquations(tVal, dVals,
    ...
    rxnRates, enzymeProdRates, enzymeSuppRates,
    TotalRNAP, RNaseH, RNaseA,
    tempDiff=0, flReader="CYT5"
):
    ...
    freeAntiDNARep = dVals[4*oceCount : 5*oceCount] # Free Anti-DNA Repressor

    BlkCoactComp   = dVals[5*oceCount : 6*oceCount] # Blocker-coactivator complex
    ActRepressComp = dVals[6*oceCount : 7*oceCount] # Activator-repressor complex
    CSilenceComp   = dVals[7*oceCount : 8*oceCount] # Coactivator-C-Silencer complex
    RSilenceComp   = dVals[8*oceCount : 9*oceCount] # Repressor-R-Silencer complex
    AntiDNARepComp = dVals[9*oceCount :10*oceCount] # Anti DNA Repressor-DNA Repressor complex

    ActDNARepComp = dVals[10*oceCount : 11*oceCount]

    btmIndex = 11*oceCount
    ...
    btmIndex = 11*oceCount+2*ngCount
    DeadRNAP   = dVals[btmIndex]
    DTT        = dVals[btmIndex + 1]
    freeAptlet = dVals[btmIndex + 2 : btmIndex+oceCount+len(nCParsed["A"])] # Aptlet unbound from corresponding protein


    # Some helpful components to compress the math (not part of the list of diff eqs)
    RNAP = TotalRNAP - DeadRNAP

    freeBlocker = classProperties["G"]["Blocker"] - BlkCoactComp - inputMat.T@BlkNGComp
    freeActivator = classProperties["G"]["Activator"] - ActRepressComp - ActDNARepComp - inputMat.T@ActNGComp
    freeDNARep = classProperties["G"]["DNARepressor"] - AntiDNARepComp - ActDNARepComp
    ...
    actDNARepress = rxnRates["ActDNARep"]*freeActivator*freeDNARep
    actDNARepressDisplace = freeDNARep*(inputMat.T@(rxnRates["ActDNARepDisplacement"]*ActNGComp))

    antiRepressing = rxnRates["AntiRepressing"]*freeDNARep*freeAntiDNARep
    antiRepressDisplace = rxnRates["AntiRepressDisplace"]*freeAntiDNARep*ActDNARepComp
    ...
    # Add enzyme activity dynamics
    # Eyring equation cannot correct for beginning plate reader dynamics without delta_G of enzyme rxn, removed from code
    # Assuming Michaelis-Menten kinetics for enzyme-facilitated transcription
    rnaseACoef = rxnRates["RNaseADegrade"]*RNaseA # Degrades RNA in solution
    rnaseHCoef = rxnRates["RNaseHDegrade"]*RNaseH # RNase H degrades RNA in RNA-DNA duplexes.
    outputProduction = {} # 

    normalizedByK_Ms = ActNGComp/enzymeProdRates["K_M"]
    resultingProd = enzymeProdRates["RNAPProdRate"]*RNAP*normalizedByK_Ms/(1+sum(normalizedByK_Ms))

    for outputType in geneletOutputs:
        outputProduction[outputType] = conMats["G"][:, geneletOutputIndex[outputType], :].T@(resultingProd)
    ...
    revivedRNAP = 0 if "Const" in enzymeSuppRates else enzymeSuppRates["RNAPRevival"]*DeadRNAP*DTT


    # Assemble the next iteration of diff eq's
    #TODO where did the RNAP*rxnRates variables go?
    # Line below is old equation from oldGeneletModel
    #dFreeCoact = -blkCoactDisplace - netBlkCoact - netCSilencing - rnaseACoef*freeCoact + RNAP*rxnRates["CoactivatorP"] * outputProduction["C"] +  rxnRates["autoActivateFreeBlocker"] * freeBlocker + rxnRates["autoActivateBlkCoactComp"] * BlkCoactComp + rxnRates["autoActivateBlkNGComp"]*(inputMat.T@BlkNGComp)
    dFreeCoact = -blkCoactDisplace - netBlkCoact - netCSilencing - rnaseACoef*freeCoact + outputProduction["C"]

    # Modified dFreeRepress for autoInhibition.
    dFreeRepress = -actRepressDisplace - netActRepress - netRSilencing - rnaseACoef*freeRepress + outputProduction["R"] + rxnRates["AutoInhib-Free-Act"] * freeActivator + rxnRates["AutoInhib-Act-Repressor"] * ActRepressComp + rxnRates["AutoInhib-Act-Genelet"]*(inputMat.T@ActNGComp)
    dFreeCSilencer = -netCSilencing - rnaseACoef*freeCSilencer + outputProduction["CS"]
    dFreeRSilencer = -netRSilencing - rnaseACoef*freeRSilencer + outputProduction["RS"]
    dFreeAntiDNARep = -antiRepressing - antiRepressDisplace - rnaseACoef*freeAntiDNARep + outputProduction["AntiR"]

    dBlkCoactComp = netBlkCoact + blkCoactDisplace - rnaseHCoef*BlkCoactComp
    dActRepressComp = netActRepress + actRepressDisplace - rnaseHCoef*ActRepressComp
    dCSilenceComp = netCSilencing - rnaseACoef*CSilenceComp
    dRSilenceComp = netRSilencing - rnaseACoef*RSilenceComp
    dAntiDNARepComp = antiRepressing + antiRepressDisplace - rnaseHCoef*AntiDNARepComp

    dActDNARepComp = actDNARepress + actDNARepressDisplace - antiRepressDisplace
    ...
    dActNGComp = -rxnRates["ActRepressDisplacement"]*ActNGComp*(inputMat@freeRepress) - rxnRates["ActDNARepDisplacement"]*ActNGComp*(inputMat@freeDNARep) - ActNGToBlkNG + rxnRates["ActivatorNode"]*freeNG*(inputMat@freeActivator) - rxnRates["ActivatorNodeReverse"]*ActNGComp
    
    dDeadRNAP = 0 if "Const" in enzymeSuppRates else np.array([-revivedRNAP + enzymeSuppRates["RNAPDeath"]*RNAP])
    dDTT = 0 if "Const" in enzymeSuppRates else np.array([-revivedRNAP - enzymeSuppRates["DTTDeath"]*DTT])
    ...
    global specT
    tValMinutes = tVal/60
    if tValMinutes - specT >= 2:
        # Print item(s) of interest
        # print(GU.roundToSigFig(tValMinutes, 6), )
        specT = tValMinutes
    

    return np.concatenate([dFreeCoact, dFreeRepress, dFreeCSilencer, dFreeRSilencer, dFreeAntiDNARep, dBlkCoactComp, dActRepressComp, dCSilenceComp, dRSilenceComp, dAntiDNARepComp, dActDNARepComp, dBlkNGComp, dActNGComp, [dDeadRNAP, dDTT], dFreeAptlet])
...
class GeneletNetwork:
    ...
    def __init__(self, inputDomains, outputDomains, considerDTT=False):
        ...
        self.EnzymeProdRates = { # Set up local enzyme rates for the network
            "RNAPProdRate" : np.repeat(enzymeProdRates["RNAPProdRate"], ngCount),
            "K_M"          : np.repeat(enzymeProdRates["K_M"], ngCount)
        }

        if considerDTT:
            self.EnzymeSuppRates = {
                "RNAPDeath"   : 0.693147/enzymeSuppRates["RNAPHalfLife"], # (1/s), Calculated as ln(2)/(half life)
                "DTTDeath"    : 0.693147/enzymeSuppRates["DTTHalfLife"], # (1/s), Calculated as above
                "RNAPRevival" : enzymeSuppRates["RNAPRevival"] # Revival rate
            }
        else:
            self.EnzymeSuppRates = {
                "Const" : True
    ...
    # RNAP, RNaseH, and RNaseA: Concentrations of RNA polymerase, RNase H, and RNase A/T1 in solution respectively. They are in units of U/uL.
    # DTT: (uM) Concentration of DTT in solution before placing into the plate reader.
    # prelimTime: (s) Amount of time that passed at 37 C before RNAP was added, allowing DTT to decay.
    def setInitialConditions(self, initialState=[], nodeConcentrations=[], classProperties=[], standardBLK=True, unitChoice="nM", RNAP=3.57, RNaseH=8.92e-3, RNaseA=0, DTT=1000, prelimTime=0):
        if initialState!=[]:
            self.InitialState = initialState
        elif not hasattr(self, "InitialState"):
            sys.exit("The initial state must be defined in the first set of initial conditions.")
        ...
        if nodeConcentrations!=[]:
            ...
            sys.exit("The node concentrations must be defined in the first set of initial conditions.")
        ...
        if classProperties!=[]:
            ...
            for i, x in enumerate(self.OCEs):
                ...
                if standardBLK:
                    ...
                    sys.exit("Not enough blocker provided in solution to allow for all G"+str(x[1])+" domains to be blocked")
        elif not hasattr(self, "ClassProperties"):
            sys.exit("The class properties must be defined in the first set of initial conditions.")
        ...
        self.InitialConditions = np.concatenate([
            ...
            [0, 0 if "Const" in self.EnzymeSuppRates else 1000*DTT*np.exp(-self.EnzymeSuppRates["DTTDeath"]*prelimTime)], # Dead RNAP and starting DTT
    ...
    def simulate(self, timeList, resetRun=True, unitChoice="nM"):
        ...
        rates = lambda tVal, dVals: GenerateGeneletEquations(tVal, dVals,
            ...
            self.RxnRates, self.EnzymeProdRates, self.EnzymeSuppRates,
        ...
        if resetRun or not hasattr(self, "TimeList"):
            self.TimeList = rawSolution.t # Timestamps of saved data
        else:
            self.TimeList = np.append(self.TimeList[:-1], rawSolution.t)
        ...
        for i in range(oceCount):
            ...
            GU.safeAppend(self.OutputConcentrations, "AntiR"+oceNum,   rawSolution.y[4*oceCount + i], 1)
            GU.safeAppend(self.OutputConcentrations, "Blk-C"+oceNum,   rawSolution.y[5*oceCount + i], 1)
            GU.safeAppend(self.OutputConcentrations, "Act-R"+oceNum,   rawSolution.y[6*oceCount + i], 1)
            GU.safeAppend(self.OutputConcentrations, "CS-C"+oceNum,    rawSolution.y[7*oceCount + i], 1)
            GU.safeAppend(self.OutputConcentrations, "RS-R"+oceNum,    rawSolution.y[8*oceCount + i], 1)
            GU.safeAppend(self.OutputConcentrations, "AntiR-R"+oceNum, rawSolution.y[9*oceCount + i], 1)
            GU.safeAppend(self.OutputConcentrations, "Act-DNAR"+oceNum, rawSolution.y[10*oceCount + i], 1)

        for i in range(ngCount):
            cP = self.DomainList["G"][i] # Class pair list
            ngName = cP[0][0]+str(cP[0][1])+"->"+cP[1][0]+ ("" if cP[1][0]=="" else str(cP[1][1]))
            GU.safeAppend(self.OutputConcentrations, "Blk: "+ngName, rawSolution.y[11*oceCount +           i], 1)
            GU.safeAppend(self.OutputConcentrations, "Act: "+ngName, rawSolution.y[11*oceCount + ngCount + i], 1)

        aptletCount = len(self.DomainList["A"])
        for i in range(aptletCount):
            aptNum = str(self.DomainList["A"][0][i][1])
            GU.safeAppend(self.OutputConcentrations, "Apt"+aptNum, rawSolution.y[11*oceCount+2*ngCount + i], 1)
    ...
    def modifyRate(self, rateType, newValue, target=None):
        if rateType in self.EnzymeProdRates:
            self.EnzymeProdRates[rateType][target] = newValue
        elif rateType in self.EnzymeSuppRates:
            self.EnzymeSuppRates[rateType] = newValue
        elif rateType=="RNaseA":
            self.RxnRates["RNaseADegrade"] = np.full(len(self.DomainList["G"]), newValue)
        elif rateType=="Activator" or rateType=="Blocker":
            self.RxnRates[rateType+"Node"] = np.full(len(self.DomainList["G"]), newValue)
        
        elif rateType=="AutoInhib-Act-Repressor":
            self.RxnRates[rateType][target] = newValue
        elif rateType=="AutoInhib-Free-Act":
            self.RxnRates[rateType][target] = newValue
        elif rateType=="AutoInhib-Act-Genelet":
            self.RxnRates[rateType][target] = newValue
    ...
    def saveGeneletParams(self, fileName="plotFile", timeStamp=True):
        paramsTxt = ''.join([
            ...
            "Anti-Repressor Conc. (nM):"+str(self.ClassProperties["G"]["DNARepressor"]),
